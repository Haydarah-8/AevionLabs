...
export function Footer() {
  ...
  const y = useTransform(progress, [0, 1], isDesktop ? ["-10%", "0%"] : ["0%", "0%"]);
  const overlayOpacity = useTransform(progress, [0, 1], isDesktop ? [0.375, 0] : [0, 0]);
        ...
        <div className="footer_main_wrap grid-col-12">
          <div className="footer_top_wrap grid-col-12">
            <div className="footer_links_wrap">
              ...
              {NAV.map((l) => (
                <TransitionLink key={l.href} href={l.href} className="footer_link w-inline-block">
                  <HoverGroup>
                    <SlideUpHover>
                      <span className="footer_link_text u-text-md">{l.label}</span>
                    </SlideUpHover>
                  </HoverGroup>
                </TransitionLink>
              ))}
              ...
              {SOCIAL_LINKS.length
                ? SOCIAL_LINKS.map((s) => (
                    ...
                    >
                      <HoverGroup>
                        <SlideUpHover>
                          <span className="footer_link_text u-text-md">{s.label}</span>
                        </SlideUpHover>
                      </HoverGroup>
                    </a>
                  ))
                : SERVICE_LINKS.map((s) => (
                    ...
                    >
                      <HoverGroup>
                        <SlideUpHover>
                          <span className="footer_link_text u-text-md">{s}</span>
                        </SlideUpHover>
                      </HoverGroup>
                    </TransitionLink>
                  ))}
            ...
            <div className="footer_email_wrap">
              <div className="footer_email_text u-text-md">
                AEVION LABS designs and builds websites: strategy, interface and
                engineering handled by one team, start to launch.
              </div>
              <div className="footer_link_cpt u-text-sm is-left">Contact us</div>
              <LinkBtn
                onClick={openModal}
                className="footer"
          ...
          <div className="footer_bottom_wrap grid-col-12">
            ...
            <div className="footer_bottom_text_wrap">
              <p className="footer_bottom_text u-text-base">
                Web design and development for teams who care how it works, not just
                how it looks. Start a project:
              </p>
              <LinkBtn
                href={`mailto:${CONTACT_EMAIL}`}
            ...
            <div className="footer_bottom_year u-text-base">© 2026 Aevion Labs</div>
          </div>
        </div>
      </div>
      <motion.div className="footer_overlay" style={{ opacity: overlayOpacity }} />
    </motion.div>
    </div>
  );
}
