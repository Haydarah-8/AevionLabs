...
/* GSAP-equivalent cubic-bezier easings used across the public site */
export const EASE = {
...
/** Use with next/image `unoptimized` for SVGs (and Simple Icons URLs) so the optimizer does not reject image/svg+xml. */
export function imageSrcNeedsUnoptimized(src: string): boolean {
  return /\.svg(\?|$)/i.test(src) || src.includes("simpleicons.org");
}
